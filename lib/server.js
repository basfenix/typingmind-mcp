...
const createRemoteClient = async ({ clientId, url, headers = {}, env = {} }) => {
  let client = undefined;
  const baseUrl = new URL(url);
  
  // Convert API_KEY from env to Authorization header
  if (env.API_KEY && !headers.Authorization) {
    headers.Authorization = `Bearer ${env.API_KEY}`;
    console.log(`Converting API_KEY from env to Authorization header for client ${clientId}`);
  }
  
  console.log(`Setting headers for client ${clientId}:`, headers);
  ...
  try {
    ...
    // FIXED: Use requestInit.headers instead of headers
    const transport = new StreamableHTTPClientTransport(baseUrl, {
      requestInit: {
        headers: headers  // <-- This is the correct way!
      }
    });
    await client.connect(transport);
    console.log('Connected using Streamable HTTP transport with headers:', Object.keys(headers));
  } catch (error) {
    ...
    console.log('Connected using SSE transport with headers:', Object.keys(headers));
  }
...
async function startClient(clientId, config) {
 ...
 // NEW CODE:
  ...
  client = await createRemoteClient({ 
    clientId, 
    url, 
